...
def send_request(site_url):
    while True:
        try:
            ...
            response = requests.get(site_url, proxies=proxy_dict, auth=auth, headers=headers, timeout=5)
...
def start_threads():
    ...
    # Start a large number of threads for concurrent requests
    num_threads = 100  # Number of threads to start
